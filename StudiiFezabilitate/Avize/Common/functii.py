...
def genereaza_document_final_EE_Delgaz(lucrare, cerere_pdf_path, cu, beneficiar, temp_dir):
    """
    Combină toate fișierele și pregătește documentul final pentru a fi livrat
    """
    path_document_final = os.path.join(
        temp_dir, f"Documentatie aviz EE Delgaz {lucrare.judet.nume} - {beneficiar.nume}.pdf"
    )
...
def genereaza_email_EE_Delgaz(lucrare, beneficiar, cu, contact, temp_dir):
    """
    ...
    email_choices = {
        'Iași': "energie.electrica_iasi@delgaz-grid.ro",
        'Neamț': "energie.electrica_neamt@delgaz-grid.ro",
        'Suceava': "energie.electrica_suceava@delgaz-grid.ro",
        'Botoșani': "energie.electrica_botosani@delgaz-grid.ro",
        'Vaslui': "energie.electrica_vaslui@delgaz-grid.ro",
        'Bacău': "energie.electrica_bacau@delgaz-grid.ro",
    }

    email = email_choices.get(lucrare.judet.nume)

    context_email = {
        'email_EE_Delgaz': email,
        'nume_beneficiar': beneficiar.nume,
        'nr_cu': cu.numar,
        'data_cu': cu.data,
        'emitent_cu': cu.emitent,
