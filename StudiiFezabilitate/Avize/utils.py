...
def genereaza_readme(temp_dir, model_detalii):

    readme_pdf_path = x.copy_doc_to_pdf(model_detalii, temp_dir)
...
def genereaza_document_final_print(avizCU, cerere_pdf_path, cu, beneficiar, temp_dir):
    ...
    """
    path_document_final = os.path.join(
        temp_dir, f"Documentatie {avizCU.nume_aviz.nume} - {beneficiar.nume} - de printat.pdf"
    )
